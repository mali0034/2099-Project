...
public class Location implements Printable {
	...
	public void tick() {
		...
		for(Item item :  new ArrayList<>(items)) {
			item.tick(this);
			if (item instanceof Fruit && ((Fruit) item).getAge()>=15){
				this.removeItem(item);
			}
			if (item instanceof StegosaurCorpse && ((StegosaurCorpse) item).getAge()>=20){
				this.removeItem(item);
			}
			else if (item instanceof BrachiosaurCorpse && ((BrachiosaurCorpse) item).getAge()>=40){
				this.removeItem(item);
			}
			else if (item instanceof AllosaurCorpse && ((AllosaurCorpse) item).getAge()>=20){
				this.removeItem(item);
			}
			else if (item instanceof PterodactylCorpse && ((PterodactylCorpse) item).getAge()>=20){
				this.removeItem(item);
			}
			else if (item instanceof AllosaurEgg && ((AllosaurEgg) item).getAge()>=2){
				this.addActor(new Allosaur("Allosaur",genders[rand.nextInt(1)],0));
                player.addEcoPoints(1000);
				this.removeItem(item);
			}
			else if (item instanceof BrachiosaurEgg && ((BrachiosaurEgg) item).getAge()>=35){
				this.addActor(new Brachiosaur("Brachiosaur",genders[rand.nextInt(1)],0));
                player.addEcoPoints(1000);			
				this.removeItem(item);
			}
			else if (item instanceof StegosaurEgg && ((StegosaurEgg) item).getAge()>=25){
                this.addActor(new Stegosaur("Stegosaur",genders[rand.nextInt(1)],0));
                player.addEcoPoints(100);
                this.removeItem(item);
			}
			else if (item instanceof PterodactylEgg && ((PterodactylEgg) item).getAge()>=2){
                this.addActor(new Pterodactyl("Pterodactyl",genders[rand.nextInt(1)],0));
                player.addEcoPoints(100);
                this.removeItem(item);
			}
